...
if [ $# != 3 ]; then
   echo "usage: steps/align.sh <wav> <txt> <result_ctm>"
   exit 1;
fi
...
# Extract the name of the file, and its path
dname=$(basename $srcwav)
path=$(dirname $srcwav)
id=${dname::-4}
...
mkdir -p $dir/data
echo "$id sox $srcwav -c 1 -2 -t wav - rate -v 16k |" > $dir/data/wav.scp

cat ${srctxt} | uconv -x any-nfc | perl -npe 's/[\r\n]/ /g; s/[,.!?:]//g;' | perl -npe "s/^/$id /" > $dir/data/text
echo "$id $id" > $dir/data/spk2utt
echo "$id $id" > $dir/data/utt2spk
